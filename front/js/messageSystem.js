			...
			function (tx, result) { //执行成功的回调函数
				...
				//查看已有的联系人列表中是否有目标联系人
					if(isExist){
						...
						//将此联系人添加到最近联系人列表顶部
						var new_html = "<a href='javascript:void(0)' onclick='openSingleChat("+newTarget+",this)'><li><div class='userFace'><img src='../"+newTargetFace+"' /></div><strong>"+newTargetName+"</strong><p>"+""+"</p><i class='delete_msg' id='"+newTarget+"'>删除</i></li></a>";
						$(".msg_list ul").prepend(new_html);
					}
...
function insert_list(userId, targetId, targetName, targetFace, lastTime, lastMsg){
	dataBase.transaction(function (tx) {
		tx.executeSql(
			"insert into recent_msg_list (userId, targetId, targetName, targetFace, lastTime, lastMsg) values(?, ?, ?, ?, ?, ?)",
			[userId, targetId, targetName, targetFace, lastTime, lastMsg],
			function () { 
				//alert('添加数据成功'); 
			},
			function (tx, error) { 
